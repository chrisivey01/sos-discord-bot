...
module.exports = {
    ...
    register: async (message, database, client, updateLinked, world) => {
        ...
        const sql = "INSERT INTO sos_discord SET ? ON DUPLICATE KEY UPDATE api = VALUES(api), world_id = VALUES(world_id), account = VALUES(account)";

        database.query(sql, sosData, (err, result, field) => {
            if(err){
                console.log(err)
            }else{
                message.channel.send("You've been added to the Sea of Sorrows discord.")
        ...
        const verifiedRole = server.roles.find(role => role.name === "SoS Citizen");
        if(response.data.access[0] === "PlayForFree"){
            await message.channel.send("You're a free player, buy the game with ya broke ass!")
        }else if(response.data.world === world || response.data.world === updateLinked ){
            await pendingUser.addRole(verifiedRole.id)
            await message.channel.send("You're now verified!")
        }else{
            await message.channel.send("Your account appears to not be on Sea of Sorrows or its link, or does not have a level 60+ character capable of entering WvW. Please confirm you can WvW on a character on FA or its link before reattempting to verify.")
        }

        }catch(err){
            await client.guilds.get("315306476212322315").channels.get("315553480418787329").send("User --> " + message.author.username + " is having issues registering. " +
